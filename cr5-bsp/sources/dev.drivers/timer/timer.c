...
uint32 read_timer(void) {
    uint32 reg_base = MCU_BSP_TIMER_BASE + (2UL * 0x100UL);
    uint32 reg_main_counter = reg_base + TMR_MAIN_CNT;
    return SAL_ReadReg(reg_main_counter);
}

// volatile bool delay_completed = false;

// static sint32 TimerDelayHandler(TIMERChannel_t channel, void *pArgs) {
//     (void)channel;
//     (void)pArgs;

//     delay_completed = true;
//     (void)GIC_IntSrcDis((uint32)GIC_TIMER_1);
//     mcu_printf("gic dis excuted\n");
//     (void)TIMER_Disable(channel);
//     mcu_printf("ch dis excuted\n");
