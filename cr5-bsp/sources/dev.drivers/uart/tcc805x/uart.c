...
)
{
    ...
    if (ucCh < UART_CH_MAX)
    {
        ...
        // 세마포어를 사용해 태스크를 깨움
        // ISR에서 세마포어 해제
        // SAL_SemaphoreRelease(xUARTSemaphore);
        // SAL_SemaphoreWait(xUARTSemaphore, -1, SAL_OPT_BLOCKING);

        // (void)SAL_EventSet(gRecvFromMidTaskEventId, 0x00000001UL, SAL_EVENT_OPT_FLAG_SET);
    }
}
...
)
{
    ...
    else
    {
        ...
        for(i = 0 ; i < cnt ; i++)
        {
            // mcu_printf("[%d] %04x \n", uart[ucCh].sRxIntr.iTail, UART_RegRead(UART_CH2, UART_REG_IFLS));
...
)
{
    ...
    if (uartStat != NULL_PTR)
    {
        ...
        if (status != 0UL)
        {
            ...
            (void)SAL_EventSet(gRecvFromMidTaskEventId, 0x00000002UL, SAL_EVENT_OPT_FLAG_SET);
        }
    }
}
