        ...
        if(isOpen == 1U)
        {
            switch ((IPCIoCmd_t) uiOpt)
            {
                case IOCTL_IPC_WRITE:
                {
                    ...
                    {
                        //mcu_printf("[MBOX_TEST] IOCTL_IPC_WRITE : error\n");
                        //if(pPar1 == NULL_PTR) mcu_printf("[MBOX_TEST] cmd NULL\n");
                        //if(pPar2 == NULL_PTR) mcu_printf("[MBOX_TEST] cmd NULL\n");
...
{
    int32 lRet;
    uint32 siCh = IPC_SVC_CH_CA72_NONSECURE;
    uint32 uiOptions = IPC_MODE_0;
    ...
    if(lRet == IPC_SUCCESS)
    {
        ...
        // open driver
        IPC_Main_Initialize((IPCSvcCh_t) siCh);
...
int32 IPC_SendToMainCtlCmdHandler
(
    IPCCmdId_t                          cmdID
)
{
    ...
    IPC_WriteLock(siCh);
    ret = IPC_SendCtl(IPC_SVC_CH_CA72_NONSECURE, cmdID);
    IPC_WriteUnlock(siCh);

    return ret;
}

int32 IPC_SendToMainDataHandler
(
    uint32                               data
)
{
    ...
    IPC_WriteLock(siCh);
    ret = IPC_SendData(IPC_SVC_CH_CA72_NONSECURE, data);
    IPC_WriteUnlock(siCh);
...
int32 IPC_apiCreateTask(void)
{
    return IPC_ReceiveTaskCreate();
}
#endif