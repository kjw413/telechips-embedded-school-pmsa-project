...
uint8   send_data[MID_TX_BUFFER_SIZE] = {ID_RED, 0, 0, 0};

static void IPC_ReceiveDetectData
(
    IPCSvcCh_t                          siCh,
    const IPCMboxMsgReg_t  *            pMsg
)
{
    // mcu_printf("[MBOX_TEST] IPC_ReceiveDetectData IN\n", pMsg->mmDat);
    ...
    if(pMsg != NULL_PTR)
    {
        ...
        if(gAutoholdMode){
            ...
            mcu_printf("[MBOX_TEST][SEND_TO_MID] led light: %d (0: false, 1: true)\n", pMsg->mmDat[0]);

        }
        else{
            mcu_printf("[MBOX_TEST][NO_SEND] led light: %d (0: false, 1: true)\n", pMsg->mmDat[0]);
        }

    }

    // mcu_printf("[MBOX_TEST] IPC_ReceiveDetectData OUT\n", pMsg->mmDat);
    ...
    while(1)
    {
        ...
        if(ret >= 0)
        {
            ...
            switch((IPCCmdType_t) cmdType)
            {
                case CTL_CMD :
                {
                    //mcu_printf("[MBOX_TEST] IPC_ControlTaskA72NonSecure: CTL_CMD\n");
                ...
                // case WRITE_CMD :
                // {
                //     //mcu_printf("[MBOX_TEST] IPC_ControlTaskA72NonSecure: WRITE_CMD\n");
...
#if defined(CONFIG_FRONT) && CONFIG_FRONT == 1
static void IPC_ReceiveTaskA72
(
    void *                              p_arg
)
{
...
int32 IPC_ReceiveTaskCreate(void)
{
    IPCSvcCh_t siCh = IPC_SVC_CH_CA72_NONSECURE;
    int32 ret;

    ret = (int32)SAL_TaskCreate
    (
        &(gIPCCtrTaskInfo[siCh].ipcControlTaskId),
        (const uint8 *) "IPC_ReceiveTaskA72",
        (SALTaskFunc) &IPC_ReceiveTaskA72,
        (uint32 * const) &(gIPCCtrTaskInfo[siCh].ipcControlTaskStk[0]),
        IPC_TASK_START_STK_SIZE,
        3, NULL_PTR
    );
