...
{
    IPC_CreateIPCParserTask(); // ipc_api.h  (by jjung)
}
...
int32 IPC_init()
{
    return IPC_Main_Open();
}

int32 IPC_SendToMainCtlCmd
(
    IPCCmdId_t                          cmdID
)
{
    ...
    if (cmdID <= START_CTL || cmdID >= END_CTL)
    {
        ret = IPC_ERR_ARGUMENT;
    }
    else
    {
        ret = IPC_SendToMainCtlCmdHandler(cmdID);
    }

    return ret;
}

int32 IPC_SendToMainData
(
    uint32                               data
)
{
    ...
    ret = IPC_SendToMainDataHandler(data);
