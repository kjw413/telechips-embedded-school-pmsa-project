...
)
{
    ...
    while (1)
    {
        ...
        // mcu_printf("[IPC_MainParserTask()] ret = %d\n", ipc_ret);

        (void) SAL_TaskSleep(10);
    }
}

// static void IPC_MainReceiveDataTask
// (
//     void *                              pArg
// )
// {
...
//     while (1)
//     {
//         if(cbFunc[IPC_SVC_CH_CA53_NONSECURE][DATA_CMD].ipcCbFunc != NULL_PTR) // null pointer checking
//         {
//             cbFunc[IPC_SVC_CH_CA53_NONSECURE][cmd1].ipcCbFunc(cmd2, &packetReceive[siCh][9], dataLength);
//         }
...
)
{
    ...
    // // TCC805x -> IPC_SVC_MAX == 4 (by jjung)
...
#if defined(TCC805x)
    // IPC_Open() -> IPC_Initialize() -> Mbox_devOpen()  by jjung
