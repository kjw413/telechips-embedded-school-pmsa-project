...
)
{
    ...
    if(pMsg != NULL_PTR)
    {
        ...
        if(pMsg->mmDat[0] != detect_flag)
        {
            // add code for change led light detect value
            // light detect value == pMsg->mmDat[0]
            mcu_printf("[MBOX_TEST] detect led light: %d (0: false, 1: true)\n", pMsg->mmDat[0]);
            detect_flag = pMsg->mmDat[0];
        }
    }
    ...
    while(1)
    {
        ...
        if(ret >= 0)
        {
            ...
            switch((IPCCmdType_t) cmdType)
            {
                ...
                case DATA_CMD:
                {
                    // mcu_printf("[MBOX_TEST] IPC_ControlTaskA72NonSecure: DATA_CMD\n");
