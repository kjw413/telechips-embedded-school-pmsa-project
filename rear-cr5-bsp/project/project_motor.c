...
static void GPIO_Set_Switch(void);
static void PWM_Setting(void);
static void Motor_Init(PDMModeConfig_t *ModeConfigInfo);
static void Motor_UpdateDuty(PDMModeConfig_t *ModeConfigInfo, uint32 x);
static void Motor_HandleStateChange(uint8 *prev_state, uint32 *x);
static void Motor_ConfigurePDM(PDMModeConfig_t *ModeConfigInfo);
...
static void Motor_Init(PDMModeConfig_t *ModeConfigInfo) { // PDM 설정
...
static void Motor_UpdateDuty(PDMModeConfig_t *ModeConfigInfo, uint32 x) { // DutyCycle, Period 설정 
...
static void Motor_HandleStateChange(uint8 *prev_state, uint32 *x) {
    uint8 local_motor_state = motor_state;

	if (autohold == 1 && *x == 0) {  // Autohold 활성화 상태이고, 																	        // DutyCycle이 0인 상태(정지상태)이면 주행 x
        GPIO_Set(IN_2_4, 0UL);
        GPIO_Set(IN_1_3, 0UL);
        mcu_printf("Auto-Hold active. Motor stopped.\n");
        *x = 0;
        return;  
    	}
...
static void Motor_ConfigurePDM(PDMModeConfig_t *ModeConfigInfo) { // PDM 활성화
...
static void Motor(void *pArg) {
    ...
    while (1) {
        ...
        // x 값 업데이트, 일반적으로 D에 넣으면 그냥 굴러가는거
        x += 4;
