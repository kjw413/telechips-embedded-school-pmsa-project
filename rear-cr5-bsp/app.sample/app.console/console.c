...
#include "/home/junghyun/topst/cr5-bsp/sources/project/project_ipc.h"
...
static void CSL_DeviceMbox(uint8 ucArgc, void *pArgv[]) {
    int32 ipc_ret;
    uint32 data = 0; // 총 4바이트 데이터 배열
    uint16 number;       // 2바이트 숫자 (첫 번째 인자)
    uint8 mode;
    uint8 flag;          // 1바이트 플래그 (세 번째 인자)

    // 인자 개수 검증
    if (ucArgc < 3) {
        mcu_printf("Insufficient arguments. Expected 3 arguments.\n");
        return;
    }

    // 첫 번째 인자 파싱: 숫자 (2바이트)
    number = (uint16)atoi((char *)pArgv[0]); // 숫자로 변환
    data = (number << 16);          

    // 두 번째 인자 파싱: D/R/P 문자 (1바이트)
    mode = ((char *)pArgv[1])[0];   // 첫 번째 문자 가져오기
    data |= (uint8)mode << 8; 

    // 세 번째 인자 파싱: 플래그 (1/0)
    flag = (uint8)atoi((char *)pArgv[2]); // 플래그 값 변환
    if (flag != 0 && flag != 1) {
        mcu_printf("Invalid flag. Expected 0 or 1.\n");
        return;
    }
    data |= flag; // 1바이트로 저장

    // 첫 번째 IPC 전송
    if ((ipc_ret = ipc_send_on_sound()) < 0) {
        mcu_printf("Error in ipc_send_on_sound(): %d\n", ipc_ret);
        return;
    }

    // 두 번째 IPC 전송: 파싱된 데이터 전달
    if ((ipc_ret = ipc_send_data(data)) < 0) {
        mcu_printf("Error in ipc_send_data(): %d\n", ipc_ret);
    }
}
